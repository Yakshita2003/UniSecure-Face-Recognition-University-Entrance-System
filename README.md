...
🗄️ Database Structure (UniSecure.db)
Tables:
...
Image is captured via webcam using st.camera_input()
Facial encoding is generated using face_recognition
Encoding is matched against existing encodings stored in face_encodings.pkl
If match found → Access Granted ✅; else → Access Denied ❌
Log is saved with timestamp and status
